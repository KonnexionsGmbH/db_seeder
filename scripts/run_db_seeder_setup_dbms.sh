...
if [ "$DB_SEEDER_DBMS" = "derby" ]; then
    ...
    docker create --name db_seeder_db -p $DB_SEEDER_CONNECTION_HOST:$DB_SEEDER_CONTAINER_HOST/tcp konnexionsgmbh/apache_derby:$DB_SEEDER_VERSION
...
if [ "$DB_SEEDER_DBMS" = "h2" ]; then
    ...
    docker create --name db_seeder_db -p $DB_SEEDER_CONNECTION_HOST:$DB_SEEDER_CONTAINER_HOST/tcp konnexionsgmbh/h2_database_engine:$DB_SEEDER_VERSION
...
if [ "$DB_SEEDER_DBMS" = "informix" ]; then
    ...
    docker create --name db_seeder_db -e LICENSE=accept -e DB_INIT=1 -p $DB_SEEDER_CONNECTION_HOST:$DB_SEEDER_CONTAINER_HOST --privileged ibmcom/informix-developer-database:$DB_SEEDER_VERSION
...
if [ "$DB_SEEDER_DBMS" = "mimer" ]; then
    ...
    docker create --name db_seeder_db -e MIMER_SYSADM_PASSWORD=mimer -p $DB_SEEDER_CONNECTION_HOST:$DB_SEEDER_CONTAINER_HOST/tcp mimersql/mimersql_v11.0:$DB_SEEDER_VERSION
...
if [ "$DB_SEEDER_DBMS" = "mssqlserver" ]; then
    ...
    docker create --name db_seeder_db -e "ACCEPT_EULA=Y" -e "SA_PASSWORD=mssqlserver_2019" -p $DB_SEEDER_CONNECTION_HOST:$DB_SEEDER_CONTAINER_HOST mcr.microsoft.com/mssql/server:$DB_SEEDER_VERSION
...
if [ "$DB_SEEDER_DBMS" = "oracle" ]; then
    ...
    docker create --name db_seeder_db -e ORACLE_PWD=oracle -p $DB_SEEDER_CONNECTION_HOST:$DB_SEEDER_CONTAINER_HOST/tcp --shm-size 1G konnexionsgmbh/$DB_SEEDER_VERSION
...
if [ "$DB_SEEDER_DBMS" = "postgresql" ]; then
    ...
    docker create --name db_seeder_db -e POSTGRES_DB=kxn_db_sys -e POSTGRES_PASSWORD=postgresql -e POSTGRES_USER=kxn_user_sys -p $DB_SEEDER_CONNECTION_HOST:$DB_SEEDER_CONTAINER_HOST postgres:$DB_SEEDER_VERSION
