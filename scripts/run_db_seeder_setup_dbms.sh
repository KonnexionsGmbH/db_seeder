...
if [ "$DB_SEEDER_DBMS" = "h2" ]; then
    ...
    docker create --name db_seeder_db -p 9092:9092/tcp konnexionsgmbh/h2_database_engine:$DB_SEEDER_VERSION
...
if [ "$DB_SEEDER_DBMS" = "hsqldb" ]; then
    ...
    docker create --name db_seeder_db -p 9001:9001/tcp konnexionsgmbh/hypersql_database:$DB_SEEDER_VERSION
...
if [ "$DB_SEEDER_DBMS" = "informix" ]; then
    ...
    docker create --name db_seeder_db -e LICENSE=accept -e DB_INIT=1 -p 9088:9088 --privileged ibmcom/informix-developer-database:$DB_SEEDER_VERSION
